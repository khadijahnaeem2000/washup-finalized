            ...
            <td style="width:15%">
                    ...
                    if($i==1){?>
                        <script type="text/javascript">
                            ...
                            $(document).ready(function(){
                                ...
                                var disableSpecificDates = fetch_holidays_hfq() ;
                                $('.dpicker').datepicker({
                                    startDate: new Date(),
                                    format: 'yyyy/mm/dd',
                            ...
                            // fetching holidays  
                            function fetch_holidays_hfq(){
                                var hDays       = new Array();
                                var holidays    = {!! json_encode($holidays->toArray()) !!};
                                holidays.forEach(function(rec,index) {
                                    hDays[index] = format_date_hfq(new Date(rec.holiday_date)); 
                                }); 
                                return hDays;
                            }

                            function fn_delivery_date_hfq(pickup_date,inc,id){
                                this.incre= 0;
                                for(var i=1; i<=inc; i++){
                                    this.incre++;
                                    calc_delivery_date_hfq(pickup_date,this.incre,id);
                                    
                                }

                            }
                            
                            // calc_delivery_date_hfq(new Date());
                            function calc_delivery_date_hfq(pickup_date,incre,id){
                ...
                @if(isset($value->pickup_date))
                    @if($value->status_id==1)
                        {!! Form::text('pickup_date['.$value->id.']', $value->pickup_date, array('id'=>'pickup_date_'.$value->id,'readonly'=>'true','class' => 'form-control dpicker','required','style'=>'width: max-content;padding: 0 5px;','onchange'=>'fn_delivery_date_hfq(this.value,3,'.$value->id.')')) !!}
                    @else
                    ...
                    <div class="dropdown dropdown-inline">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right" style="">
                            ...
                            <ul class="navi navi-hover py-5">
                                ...
                                <li class="navi-item">
                                    <a href="#" class="navi-link edit_hfq_order"   data-id="{{$value->id}}" data-toggle="modal" data-target="#edit_order">
                                        <span class="navi-icon">
