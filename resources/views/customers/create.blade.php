...
@include( '../sweet_script')
            ...
            <div class="card card-custom gutter-b example example-compact">
                ...
                {!! Form::open(array('route' => 'customers.store','method'=>'POST','id'=>'form','enctype'=>'multipart/form-data')) !!}
                    ...
                    <div class="card-body">
                        ...
                        <div class="cls_<?php echo $service_value->id?> Specialcommercial">
                        ...
                        @foreach($all_special_servicesRetail as $service_key => $service_value)
                        <div class="cls_<?php echo $service_value->id?> Specialretail"  >
                            <?php $service_id = $service_value->id;?>
                            <h4 class="card-title">Special Services ({{$service_value->name}})  </h4>
                            <div style="height:100%;max-height:400px; overflow-y:scroll; overflow-x:hidden;">
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="table-responsive" >
                                            <table id="special_serviceTable" class="table">
                                                ...
                                                <tbody>
                                                    <?php  
                                                        foreach($special_servicesRetail as $key => $value){
                                                            if($value->id ==$service_id ){?>
                                                                <tr>
                                                                    <td>
                                                                        {{ $value->item_name }}
    ...
    <script type="text/javascript">
       $(document).ready(function(){
        ...
        function show_items($id){
            if ($id == 1) {
                    ...
                    $('.Specialretail').removeClass('hide').addClass('show');
                   
                    $('.Specialcommercial').removeClass('show').addClass('hide');
                    addRowsToCommercialTbodyRetail()
                   
                } else {
                   
                    $('.readonly_class').removeAttr("readonly");
                   
                    $('.Specialretail').removeClass('show').addClass('hide');
                   
                    $('.Specialcommercial').removeClass('hide').addClass('show');
                    addRowsToCommercialTbody() 
                }


       }

       function addRowsToCommercialTbody() {
            // Get a reference to the tbody element
            var tbody = document.getElementById('serviceslist');
            var services = {!! json_encode($services) !!};
            // Clear existing rows in case you're appending new ones
            tbody.innerHTML = '';
            
            // Iterate over the services array and create HTML for each row
            services.forEach(function(service) {
                var row = '<tr>' +
                            '<td>' + service.name + '</td>' +
                            '<td>' +
                                '<input type="number" name="service_rate[' + service.id + ']" value="' + service.rate + '" placeholder="Enter rate" class="form-control readonly_class">' +
                            '</td>' +
                            '<td>' +
                                '<span class="switch switch-outline switch-icon switch-primary">' +
                                    '<label>' +
                                        '<input type="checkbox" name="service_status[' + service.id + ']" value="1" class="form-control cls_services" data-pnt="' + service.id + '" checked>' +
                                        '<span></span>' +
                                    '</label>' +
                                '</span>' +
                            '</td>' +
       ...
       function addRowsToCommercialTbodyRetail() {
            // Get a reference to the tbody element
            var tbody = document.getElementById('serviceslist');
            var services = {!! json_encode($servicesRetail) !!};
            // Clear existing rows in case you're appending new ones
            tbody.innerHTML = '';
